...
public class DispatcherController implements DisposableBean {
    ...
    public void processDo(@PathVariable("reqCmd") String reqCmd, @RequestBody byte[] contentBytes,
        ...
        try {
            ...
            if (!store.getStoreDomain().satisfies(reqCmd0000.getDomain())) {
                ...
                this.outPrintJSONString(httpServletResponse, ErrorResponse.create(100), watch, reqCmd);
                return;
            }
            ...
            case outputsOfTokenid: {
                ...
                AbstractResponse response = walletService.getOpenAllOutputsResponse(
                        (String) request.get("tokenid"), store);
                this.outPrintJSONString(httpServletResponse, response, watch, reqCmd);
            }
                break;
            case getBalances: {
                if (!userDataService.ipCheck(reqCmd, contentBytes, httprequest)) {
                    logger.debug("getBalances denied {} {}", remoteAddr(httprequest), reqCmd);
                    return;
                }
                String reqStr = new String(bodyByte, StandardCharsets.UTF_8);
                List<String> keyStrHex000 = Json.jsonmapper().readValue(reqStr, List.class);
            ...
            case getAccountBalances: {
                if (!userDataService.ipCheck(reqCmd, contentBytes, httprequest)) {
                    logger.debug("getAccountBalances denied {} {}", remoteAddr(httprequest), reqCmd);
                    return;
                }
                String reqStr = new String(bodyByte, StandardCharsets.UTF_8);
                List<String> keyStrHex000 = Json.jsonmapper().readValue(reqStr, List.class);
            ...
            case getBlockByHash: {
                ...
                if (request.get("hashHex") != null) {
                    Block block = this.blockService.getBlock(
                            Sha256Hash.wrap(request.get("hashHex").toString()), store);
                    if (block != null) {
                        if ("true".equals(request.get("text"))) {
                            this.outPrintJSONString(httpServletResponse,
                                    GetStringResponse.create(block.toString()), watch, reqCmd);
                        } else {
            ...
            case blocksFromChainLength: {
                ...
                GetBlockListResponse response = this.blockService.blocksFromChainLength(
                        Long.valueOf((String) request.get("start")),
                        Long.valueOf((String) request.get("end")), store);
                this.outPrintJSONString(httpServletResponse, response, watch, reqCmd);
            }
                break;
    ...
    private boolean checkReady(HttpServletResponse httpServletResponse, Stopwatch watch) throws Exception {
    ...
    public void register(Block block, BlockStoreInterface store) throws BlockStoreException {
        if (serverConfiguration.getMyserverblockOnly())
            blockService.insertMyserverblocks(block.getPrevBlockHash(), block.getHash(),
