 ...
 * {@link net.bigtangle.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link net.bigtangle.crypto.MnemonicCode},
 ...
 *
 * <p>This class builds on {@link net.bigtangle.crypto.DeterministicHierarchy} and
 * {@link net.bigtangle.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    protected DeterministicKeyChain(KeyCrypter crypter, KeyParameter aesKey, DeterministicKeyChain chain) {
        ...
        for (Key eckey : chain.basicKeyChain.getKeys()) {
     ...
     * The returned key can be serialized and then passed into {@link #watch(net.bigtangle.crypto.DeterministicKey)}
    ...
    public DeterministicKeyChain toDecrypted(KeyParameter aesKey) {
        ...
        for (Key eckey : basicKeyChain.getKeys()) {
            DeterministicKey key = (DeterministicKey) eckey;
            if (key.getPath().size() != getAccountPath().size() + 2) continue; // Not a leaf key.
    ...
    public boolean checkAESKey(KeyParameter aesKey) {
        ...
        checkState(getKeyCrypter() != null, "Key chain not encrypted");
        try {
            return rootKey.decrypt(aesKey).getPubKey().equals(rootKey.getPubKey());
    ...
    /* package */ List<Key> getKeys(boolean includeLookahead) {
        List<Key> keys = basicKeyChain.getKeys();
        if (!includeLookahead) {
            int treeSize = internalParentKey.getPath().size();
            List<Key> issuedKeys = new LinkedList<Key>();
            for (Key key : keys) {
     ...
     */
    public List<Key> getIssuedReceiveKeys() {
        final List<Key> keys = new ArrayList<Key>(getKeys(false));
     ...
     * @see net.bigtangle.wallet.MarriedKeyChain
     */
