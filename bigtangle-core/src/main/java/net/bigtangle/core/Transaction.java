...
public class Transaction extends ChildMessage {
	 ...
	 * tx network message.
	 */
	public static Transaction setTransaction2(NetworkParameters params, byte[] payloadBytes) throws ProtocolException {
		Transaction a = new Transaction(params);
	 ...
	 */
	public static Transaction setTransaction3(NetworkParameters params, byte[] payload, int offset)
	 ...
	 */
	public static Transaction fromTransaction6(NetworkParameters params, byte[] payload, int offset,
			@Nullable Message parent, MessageSerializer setSerializer, int length) throws ProtocolException {
		Transaction a = new Transaction(params);
		a.setValues5(params, payload, offset, setSerializer, length);
		a.setParent(parent);
		return a;
	 ...
	 */
	public static Transaction fromTransaction5(NetworkParameters params, byte[] payload, @Nullable Message parent,
			MessageSerializer setSerializer, int length) throws ProtocolException {
		Transaction a = new Transaction(params);
		a.setValues5(params, payload, 0, setSerializer, length);
		a.setParent(parent);
		return a;
	}
	...
	public void verify() throws VerificationException {
		/*
		 * if (this.getMemo() != null && this.getMemo().length() >
		 * NetworkParameters.MAX_TRANSACTION_MEMO_SIZE) throw new
		 * VerificationException("memo size too large MAX " +
		 * NetworkParameters.MAX_TRANSACTION_MEMO_SIZE);
		 * 
		 * 
		 * Check by this.getMessageSize() > NetworkParameters.MAX_DEFAULT_BLOCK_SIZE if
		 * (this.getDataClassName()!=null&& this.getDataClassName().length() >
		 * NetworkParameters.MAX_TRANSACTION_MEMO_SIZE) throw new VerificationException(
		 * "getDataClassName size too large MAX " +
		 * NetworkParameters.MAX_TRANSACTION_MEMO_SIZE);
		 * 
		 * if (this.getDataSignature()!=null && this.getDataSignature().length >
		 * NetworkParameters.MAX_TRANSACTION_MEMO_SIZE) throw new VerificationException(
		 * "getDataSignature size too large MAX " +
		 * NetworkParameters.MAX_TRANSACTION_MEMO_SIZE);
		 */
	...
	/**
	 * <p>
	 * A transaction is time locked if at least one of its inputs is non-final and
	 * it has a lock time
	 * </p>
	 *
	 * <p>
