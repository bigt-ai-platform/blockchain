...
public class Address extends VersionedChecksummedBytes {
	...
	/**
	 * An address is a RIPEMD160 hash of a public key, therefore is always 160 bits
	 * or 20 bytes.
	 */
	...
	/**
	 * Construct an address from parameters, the address version, and the hash160
	 * form. Example:
	 * <p>
	 *
	 * <pre>
	 * new Address(MainNetParams.get(), NetworkParameters.getAddressHeader(),
	...
	/**
	 * Returns an Address that represents the script hash extracted from the given
	 * scriptPubKey
	 */
	 ...
	 *               validation.
	 * @param base58 The textual form of the address, such as
	 *               "17kzeh4N8g49GFvdDzSf8PjaPfyoD1MndL".
	 * @throws AddressFormatException if the given base58 doesn't parse or the
	 *                                checksum is invalid
	 * @throws WrongNetworkException  if the given address is valid but for a
	 *                                different chain (eg testnet vs mainnet)
	 */
	public static Address fromBase58(@Nullable NetworkParameters params, String encoded) throws AddressFormatException {
		return fromBase58Version(params, params.getAddressHeader(), encoded);

	}

	public static Address fromBase58Version(@Nullable NetworkParameters params, int version, String encoded)
		...
		return new Address(params, version, bytes);
	 ...
	 * </pre>
	 */
	public static Address fromHash160(NetworkParameters params, byte[] hash160) {
	...
	/**
	 * Returns true if this address is a Pay-To-Script-Hash (P2SH) address. See also
	 * https://github.com/bitcoin/bips/blob/master/bip-0013.mediawiki: Address
	 * Format for pay-to-script-hash
	 */
	 ...
	 * NetworkParameters. If you aren't sure which network the address is intended
	 * for (eg, it was provided by a user), you can use this to decide if it is
	 * compatible with the current wallet. You should be able to handle a null
	 * response from this method. Note that the parameters returned is not
	 * necessarily the same as the one the Address was created with.
	 *
	 * @return a NetworkParameters representing the network the address is intended
	 *         for.
	 */
