...
public class Utils {
	...
	/**
	 * Returns a copy of the given byte array with the bytes of each double-word (4
	 * bytes) reversed.
	 * 
	 * @param bytes      length must be divisible by 4.
	 * @param trimLength trim output to this length. If positive, must be divisible
	 *                   by 4.
	 */
	...
	public static void writeNBytesString(DataOutputStream dos, String message)
			throws IOException, UnsupportedEncodingException {
		dos.writeBoolean(message != null);
		if (message != null) {
			dos.writeInt(message.getBytes("UTF-8").length);
	 ...
	 * bytes representing the number in big endian format (with a sign bit).
	 * 
	 ...
	 * Bit number 24 (0x800000) represents the sign of N. Therefore, N = (-1^sign) *
	 * mantissa * 256^(exponent-3).
	 * </p>
	 *
	 * <p>
	 * Satoshi's original implementation used BN_bn2mpi() and BN_mpi2bn(). MPI uses
	 * the most significant bit of the first byte as sign. Thus 0x1234560000 is
	 * compact 0x05123456 and 0xc0de000000 is compact 0x0600c0de. Compact 0x05c0de00
	 * would be -0x40de000000.
	 * </p>
	 *
	 * <p>
	 * Bitcoin only uses this "compact" format for encoding difficulty targets,
	 * which are unsigned 256bit quantities. Thus, all the complexities of the sign
	 * bit and using base 256 are probably an implementation accident.
	 * </p>
	 */
	 ...
	 * RuntimeException. The exception can never occur given the charsets US-ASCII,
	 * ISO-8859-1, UTF-8, UTF-16, UTF-16LE or UTF-16BE.
	 *
	...
	/**
	 * Attempts to parse the given string as arbitrary-length hex or base58 and then
	 * return the results, or null if neither parse was successful.
	 */
	...
	/**
	 * Checks if the given bit is set in data, using little endian (not the same as
	...
	/**
	 * Sets the given bit in data to one, using little endian (not the same as Java
	 * native big endian)
	 */
	...
	// Can't use Closeable here because it's Java 7 only and Android devices
	// only got that with KitKat.
	...
	public String findContractValue(TokenKeyValues t, String key) {
		...
		for (KeyValue kv : t.getKeyvalues()) {
