...
public class ECKey implements EncryptableItem {
    ...
    public static LazyECPoint decompressPoint(LazyECPoint point) {
        return !point.isCompressed() ? point : new LazyECPoint(decompressPoint(point.get()), false);
    }
    ...
    public static ECKey fromPrivate(BigInteger privKey, boolean compressed) {
        ...
        return new ECKey(privKey, new LazyECPoint(getPointWithCompression(point, compressed), compressed));
    }
    ...
    public static ECKey fromPublicOnly(ECPoint pub) {
        return new ECKey(null, new LazyECPoint(pub, false));
    }
    ...
    public static ECKey fromPublicOnly(byte[] pub) {
        boolean compressed = pub != null && pub.length > 0 && (pub[0] == 2 || pub[0] == 3);
    ...
    public ECKey decompress() {
        if (!pub.isCompressed())
            ...
            return new ECKey(priv, new LazyECPoint(decompressPoint(pub.get()), false));
    }
     ...
     * @throws net.bigtangle.core.ECKey.MissingPrivateKeyException if the private key is missing or encrypted.
     */
    public byte[] toASN1() {
        try {
     ...
     * usually encoded using ASN.1 format, so you want {@link net.bigtangle.core.ECKey.ECDSASignature#toASN1()}
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Bitcoin protocol, they are
     * usually encoded using DER format, so you want {@link net.bigtangle.core.ECKey.ECDSASignature#encodeToDER()}
     ...
     * @throws net.bigtangle.core.ECKey.MissingPrivateKeyException if the private key bytes are missing/encrypted.
     */
