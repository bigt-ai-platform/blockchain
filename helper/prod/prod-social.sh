...
# the local L0 requester and stream via the 3 prod Kafka mirrors:
#   kafkaeu1.bigtangle.org, kafkaeu2.bigtangle.org, kafkahk1.bigtangle.org.
#
# Usage:
...
#   IMAGE_TAG (default latest), SOCIAL_PORT (default 8091),
...
#   KAFKA_BOOTSTRAP (default the 3 prod mirrors), SERVER_APIKEY, JAVA_OPTS.
set -euo pipefail
...
KAFKA_BOOTSTRAP="${KAFKA_BOOTSTRAP:-kafkaeu1.bigtangle.org:9092,kafkaeu2.bigtangle.org:9092,kafkahk1.bigtangle.org:9092}"
...
pg_container_of() { # $1=host → running postgres container name (empty if none)
    remote "$1" "docker ps --format '{{.Names}} {{.Image}}' | awk '\$2 ~ /postgres/ {print \$1; exit}'" 2>/dev/null || true
}

ensure_db() { # $1=host — CREATE DATABASE DB_NAME when absent
    local host="$1" pgc
    pgc="$(pg_container_of "$host")"
...
start_one() { # $1=host
    local host="$1"
    log "${host}: pulling ${SOCIAL_IMAGE}:${IMAGE_TAG}"
    remote "$host" "docker pull ${SOCIAL_IMAGE}:${IMAGE_TAG} >/dev/null" \
        || die "${host}: image pull failed"
    ensure_db "$host"
    log "${host}: starting ${CONTAINER} on :${SOCIAL_PORT} (requester=${L0_REQUESTER})"
...
cmd_status() {
    ...
    for host in "${HOSTS[@]}"; do
        ...
        printf '  %-16s container=%-10s api=%s\n' "$host" "$st" "$api"
